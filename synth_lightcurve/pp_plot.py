...
BASE_DIR = f"/home/stu_jamsin/jamsin/condor_synth_1t0_no_noise_nojitter"  # change as needed
lower_bound = 0.16
higher_bound = 0.84 # for 1 sigma credible interval, but can be modified as needed
inj_number = 10
analytical = False # True if full_analysis_synth_no_resamp.py was run
...
for (param, bounds) in zip(['luminosity_distance', 'KNphi', 'KNtheta', 'log10_mej_dyn', 'log10_mej_wind', 'timeshift'], [(0, 500), (0, 90), (0, 90), (-3, -1), (-3, -0.5), (-2, 1)]):
    ...
    # resampling 
for (param, bounds) in zip(['chirp_mass', 'mass_ratio'], [(1., 2.), (0.125, 1.)]):
    fig = plt.figure(figsize=(8, 8))
    for i in range(inj_number):
        if analytical: # if full_analysis_synth_no_resamp.py was run, use the same posterior samples as for the original analysis (without resampling)
            out_file = f"{BASE_DIR}/{i}/recovered_masses{i}.csv"
            if not os.path.exists(out_file): # skip if file does not exist
                continue
            samples = pd.read_csv(out_file, sep=',')
        else:
