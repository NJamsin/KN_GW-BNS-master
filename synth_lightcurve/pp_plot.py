...
for (param, bounds) in zip(['luminosity_distance', 'KNphi', 'KNtheta', 'log10_mej_dyn', 'log10_mej_wind', 'timeshift'], [(0, 500), (0, 90), (0, 90), (-3, -1), (-3, -0.5), (-2, 1)]):
    ...
    for i in range(25):
        out_file = f"/home/stu_jamsin/jamsin/condor_synth_lc_2perday/{i}/{i}_posterior_samples.dat"
        ...
        D_L_16 = df[param].quantile(0)
        D_L_84 = df[param].quantile(1)
        # get inj val 
        inj_data = pd.read_csv(f"/home/stu_jamsin/jamsin/condor_synth_lc_2perday/{i}/true{i}.csv")
    ...
    file_path = f'/home/stu_jamsin/jamsin/condor_synth_lc_2perday/plots/{param}_injection_recovery_full.png'
...
for (param, bounds) in zip(['chirp_mass', 'mass_ratio'], [(1., 3.), (0.125, 1.)]):
    ...
    for i in range(25):
        ...
        out_file = f"/home/stu_jamsin/jamsin/condor_synth_lc_2perday/{i}/resampling/posterior_samples.dat"
        if not os.path.exists(out_file): # skip if file does not exist
            continue
        samples = pd.read_csv(out_file, delim_whitespace=True)

        D_L_median = samples[param].median()
        D_L_16 = samples[param].quantile(0)
        D_L_84 = samples[param].quantile(1)
        # get inj val 
    ...
    plt.ylabel(f'Recovered {param}')
    plt.legend()
    plt.title(f'Injection-Recovery of {param} for Manual Injections (full posterior range)')
    file_path = f'/home/stu_jamsin/jamsin/condor_synth_lc_2perday/plots/{param}_injection_recovery_full.png'
    os.makedirs(os.path.dirname(file_path), exist_ok=True)
    plt.savefig(file_path)
    plt.show()
